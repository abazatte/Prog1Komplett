...
public class Zug {
	...
	private String name;
	private Lokomotive loco;
	
	Zug(Lokomotive loco, String name) {
		this.loco = loco;
		this.name = name;
	}
	...
	void add(Wagen car) {
		...
		if(this.loco.getFirst() == null) {
			...
			Wagen current = this.loco.getFirst();
	...
	Wagen removeFirst() {
		...
		if(this.loco.getFirst().getNext() == null) {		//Wenn es nur einen Wagen gibt, den Wagen ausgeben und first auf null setzen
			Wagen ausgabe = this.loco.getFirst();
			this.loco.setFirst(null);
			return ausgabe;
		}
		else {
			Wagen ausgabe = this.loco.getFirst();				//Bei mindestens zwei Wagen wird first ausgegeben, mit next überschrieben und 
			Wagen newFirst = this.loco.getFirst().getNext();  //alle Wagen rücken eine Stelle nach vorne
	...
	void relink(Zug train) {
		Wagen current = this.loco.getFirst(); // Erster Wagen dieses Zuges
		Wagen tmp = train.loco.getFirst(); //Erster Wagen des anderen Zuges
		if(current == null) {
	...
	void revert() {
		Lokomotive loco = new Lokomotive(0, 0, null, "TempLoco");	//Lokomotive zum Tauschen
		Zug temp = new Zug(loco, "TempZug");	//Zug zum Tauschen
		Wagen current = this.loco.getFirst(); 		//Der erste Wagen des eigentlichen Zuges
